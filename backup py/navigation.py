...
def move_to_direction(current_dir: int, target_dir: int,
    ...
    # Verifica drift após avanço (acumulado de vibração, skid, etc.)
    if response == "OK":
        heading_deg, _ = imu.get_heading()
...
def _traverse_ramp(serial, imu) -> str:
    """
    Aumenta velocidade até inclinação normalizar.
    Retorna "RAMP" quando conclui (ou timeout).
    """
    incl = imu.get_inclination()
    print(f"  [RAMP] Detetada (incl={incl:.1f}°) — velocidade → {RAMP_SPEED}")
    ...
    while time.time() - start < RAMP_TIMEOUT_S:
        ...
        if inc is not None and inc >= RAMP_EXIT_DEG:
            print(f"  [RAMP] Concluída (incl={inc:.1f}°)")
            break
        time.sleep(0.05)
    else:
