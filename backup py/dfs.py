...
dfs.py — Exploração DFS iterativa com:
  - Timer de missão (para quando faltar ~30s e regressa a (0,0))
  - Backtracking físico correto (sem time.sleep arbitrário)
  - Tratamento de tiles pretos, azuis, rampas
...
def explorar_labirinto(
    ...
    Quando o deadline é atingido, para a exploração e regressa a (0,0).
    """
    ...
    # ── Estado do DFS ─────────────────────────────────────────────────────────
    stack      = [(0, 0)]       # Pilha do caminho atual (para backtracking)
    visited    = {(0, 0)}       # Células confirmadas (robot chegou lá com OK/RAMP)
    ...
    print(f"      Posição: (0,0)  Heading: {DIRECTION_NAME[heading]}")
    if mission_deadline:
    ...
    while stack:
        ...
        if mission_deadline and time.time() >= mission_deadline:
            print("\n[TIMER] Tempo esgotado! A regressar a (0,0).")
        ...
        if pos not in cell_map:
            print(f"\n{'─' * 48}")
            print(f"[POS] ({x},{y})  Heading: {DIRECTION_NAME[heading]}")
        ...
        while cell_map[pos]:
            ...
            print(f"\n  → {DIRECTION_NAME[direction]} → {next_pos}")
            ...
            # OK ou RAMP — movimento bem sucedido
            if resp == "RAMP":
                print(f"  [RAMP] Rampa atravessada → {next_pos}")
...
def _return_to_start(stack: list, heading: int, serial, imu, floor_sensor) -> int:
    """
    Percorre a pilha ao contrário até (0,0) dentro de BACKTRACK_TIMEOUT_S.
    ...
    if len(stack) <= 1:
        print("[HOME] Regressou a (0,0) com sucesso.")
    else:
        print("[HOME] Timeout — não chegou a (0,0).")
